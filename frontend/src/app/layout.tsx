...
export default function RootLayout({children}:LayoutProps<"/">){return <html lang="en" className={`${geistSans.variable} ${geistMono.variable} h-full antialiased`}><body className="min-h-full"><AuthProvider><div className="relative min-h-full"><div className="fixed right-4 top-2 z-[60] flex items-center gap-2 sm:right-5"><AccountMenu/><NotificationCenter/></div><AuthGate><LegacyMarketSelectBridge/>{children}</AuthGate><UserManagementWorkspaceGate/></div></AuthProvider></body></html>}
