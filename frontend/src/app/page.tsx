...
export default function Home() {
  ...
  function selectSymbol(value: string) { setSymbol(value); setData(null); setError(""); setLiveQuote(null); setSelectedMatchIndex(0); }
  function changeTimeframe(value: string) { setTimeframe(value); setData(null); setError(""); setLiveQuote(null); setSelectedMatchIndex(0); }
  function changePatternLength(value: string) { setPatternLength(value); setData(null); setError(""); setSelectedMatchIndex(0); }
  function changeTopK(value: string) { setTopK(value); setData(null); setError(""); setSelectedMatchIndex(0); }
  ...
  async function searchPatterns() { const requestedTopK = Number(topK); if (!Number.isInteger(requestedTopK) || requestedTopK < 5 || requestedTopK > 50) return; setLoading(true); setError(""); setData(null); setSelectedMatchIndex(0); try { const params = new URLSearchParams({ symbol, timeframe, pattern_length: patternLength, top_k: String(requestedTopK) }); const response = await fetch(`/api/backend/api/v1/pattern-search?${params.toString()}`, { cache: "no-store" }); if (!response.ok) { let message = `API returned ${response.status}`; try { const body = await response.json(); if (body?.detail) message = body.detail; } catch { /* Generic error. */ } setError(message); return; } setData((await response.json()) as SearchResponse); } catch { setError("Could not reach the Pattern Engine. Check that FastAPI is running."); } finally { setLoading(false); } }
            ...
            {data && <div className="space-y-3">
              ...
              <div ref={chartWorkspaceRef} className={`${chartsFullscreen ? "h-screen bg-[#070a0f] p-3" : ""}`}>
                <section className="grid h-full min-h-0 grid-cols-2 gap-3">
                  ...
                  <HistoricalPatternChart key={`${data.symbol}-${data.timeframe}-${data.pattern_length}`} symbol={data.symbol} timeframe={data.timeframe} patternLength={data.pattern_length} matches={data.matches} highlightLocked={highlightLocked} dashboardFullscreen={chartsFullscreen} onFullscreenToggle={() => void toggleChartsFullscreen()} selectedIndex={selectedMatchIndex} onSelectedIndexChange={setSelectedMatchIndex} />
                </section>
              </div>
              <ForwardPathChart paths={data.forward_paths} selectedMatchIndex={selectedMatchIndex} />
