...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setSelectedMatchIndex(favorite.matchIndex ?? 0);
  }, []);
  ...
  async function refreshLiveQuote() {
    try {
      ...
      setLiveQuote({ price: result.price, change: result.change_percent_24h ?? 0 });
    } catch {}
  }
  ...
  async function searchPatterns() {
    ...
    try {
      ...
      if (requestId === searchRequestRef.current && !controller.signal.aborted) {
        setData(next);
        if (favoriteMatchIndexRef.current != null) {
          setSelectedMatchIndex(Math.min(favoriteMatchIndexRef.current, Math.max(0, next.matches.length - 1)));
          favoriteMatchIndexRef.current = null;
        }
      }
    } catch (caught: unknown) {
      if (controller.signal.aborted) {
        if (requestId === searchRequestRef.current) setError("Pattern search timed out. Try again or reduce the match count.");
        return;
      }
  ...
  function pinCurrent() {
    if (!data) return;
    setPinTarget({ type: "current", symbol: data.symbol, timeframe: data.timeframe, patternLength: data.pattern_length, startTime: data.current_pattern.start_time, endTime: data.current_pattern.end_time });
  }

  function pinHistorical() {
    if (!data || !data.matches[selectedMatchIndex]) return;
    const match = data.matches[selectedMatchIndex];
  ...
  return (
    <main className="min-h-screen bg-[#070a0f] text-white">
      ...
      <PinPatternDialog favorite={pinTarget} onClose={() => setPinTarget(null)} />
    </main>
  );
}
