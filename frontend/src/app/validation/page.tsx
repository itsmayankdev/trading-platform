...
type Evaluation = {
  symbol: string; timeframe: string; checkpoints: Array<{ current_regime: string | null; same_regime_match_fraction: number | null }>;
  stability?: Record<string, Stability>;
  similarity_stability?: { median: number | null };
  regime_analysis?: { median_same_regime_match_fraction: number | null };
};
...
export default function ValidationLab() {
  ...
  async function runMatrix() {
    ...
    setResults(initial);
    await Promise.all(SYMBOLS.flatMap((symbol) => TIMEFRAMES.map(async (timeframe) => {
