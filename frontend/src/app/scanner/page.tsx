...
export default function ScannerPage() {
  ...
  return <main className="min-h-screen bg-[#070a0f] text-white">
    ...
    <div className="flex"><Sidebar symbol="ETHUSDT" collapsed={collapsed} onCollapsedChange={setCollapsed} onSymbolSelect={(symbol) => { window.location.href = `/scanner?symbol=${symbol}`; }} selectedSymbols={[]} onWatchlistToggle={() => {}} />
      <div className="min-w-0 flex-1"><div className="mx-auto max-w-[1300px] px-4 py-6 sm:px-7 lg:px-10">
        <div className="mb-6"><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-200/65">Market-wide search</div><h1 className="mt-1 text-2xl font-semibold tracking-tight">Market Scanner</h1><p className="mt-1 text-sm text-white/40">Find which core markets currently look most like something seen before.</p></div>
        <div className="mb-4 flex items-center justify-between rounded-md border border-white/7 bg-white/[0.015] px-3 py-2 text-[11px] text-white/35"><span>{TIMEFRAME} · {PATTERN_LENGTH} candles · {SYMBOLS.length} core markets</span><span>{loading ? "Scanning…" : `${ranked.length} ready`}</span></div>
        <section className="overflow-hidden rounded-lg border border-white/8 bg-[#0a0e15]">
          <div className="grid grid-cols-[48px_1.3fr_1fr_1fr_1fr] gap-4 border-b border-white/8 px-4 py-3 text-[9px] font-semibold uppercase tracking-[0.12em] text-white/25"><span>#</span><span>Market</span><span>Similarity</span><span>History at +60</span><span>Cases</span></div>
          {loading && SYMBOLS.map((symbol) => <div key={symbol} className="h-16 animate-pulse border-b border-white/6 bg-white/[0.01]" />)}
          {!loading && ranked.map((row, index) => { const response = row.response!; const top = response.matches[0]?.similarity_score ?? 0; const stat = response.statistics.find((item) => item.horizon_candles === 60); const up = (stat?.median_return ?? 0) >= 0; return <Link href={`/?symbol=${row.symbol}`} key={row.symbol} className="grid grid-cols-[48px_1.3fr_1fr_1fr_1fr] items-center gap-4 border-b border-white/6 px-4 py-4 transition last:border-0 hover:bg-white/[0.025]"><span className="font-mono text-xs text-white/25">{String(index + 1).padStart(2, "0")}</span><span><div className="text-sm font-semibold">{row.symbol.replace("USDT", "/USDT")}</div><div className="mt-0.5 text-[10px] text-white/25">{response.timeframe} · {response.pattern_length} candles</div></span><span className="font-mono text-sm font-semibold">{(top * 100).toFixed(2)}%</span><span className={`flex items-center gap-1.5 font-mono text-sm font-semibold ${up ? "text-emerald-300" : "text-rose-300"}`}>{up ? <ArrowUp size={14} /> : <ArrowDown size={14} />}{pct(stat?.median_return ?? 0)}</span><span className="font-mono text-xs text-white/45">{stat?.sample_size ?? 0}</span></Link>; })}
          {!loading && rows.filter((row) => row.error).map((row) => <div key={row.symbol} className="grid grid-cols-[48px_1.3fr_1fr_1fr_1fr] gap-4 border-b border-white/6 px-4 py-4 text-xs text-white/25"><span>—</span><span>{row.symbol.replace("USDT", "/USDT")}</span><span>Unavailable</span><span>—</span><span>—</span></div>)}
        </section>
        <div className="mt-4 text-[10px] text-white/20">Scanner ranking is based on historical pattern similarity. It does not predict which market will move next.</div>
      </div></div>
    </div>
  </main>;
}
