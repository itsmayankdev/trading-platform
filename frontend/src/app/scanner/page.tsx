...
function readScanUniverse() {
  try {
    const raw = JSON.parse(localStorage.getItem(WATCHLIST_KEY) || "[]");
    const saved = Array.isArray(raw) ? raw.filter((item): item is string => typeof item === "string") : [];
    return Array.from(new Set([...saved, readGlobalMarket().symbol])).filter(Boolean).slice(0, 20);
  } catch { return [readGlobalMarket().symbol]; }
}
...
export default function ScannerPage() {
  ...
  useEffect(() => {
    const refresh = () => setSymbols(readScanUniverse());
    refresh();
    window.addEventListener("storage", refresh);
    window.addEventListener(MARKET_CONTEXT_EVENT, refresh);
    return () => { window.removeEventListener("storage", refresh); window.removeEventListener(MARKET_CONTEXT_EVENT, refresh); };
  }, []);
  ...
  return <main className="min-h-screen bg-[#070a0f] text-white">
    ...
    <div className="flex"><Sidebar symbol={activeSymbol} collapsed={collapsed} onCollapsedChange={setCollapsed} onSymbolSelect={(symbol) => { writeGlobalMarket(symbol); window.location.href = `/scanner?symbol=${encodeURIComponent(symbol)}`; }} selectedSymbols={symbols} onWatchlistToggle={() => undefined} />
      <div className="min-w-0 flex-1"><div className="mx-auto max-w-[1400px] px-4 py-6 sm:px-7 lg:px-10">
        <div className="mb-5"><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-200/65">Market-wide search</div><h1 className="mt-1 text-2xl font-semibold tracking-tight">Market Scanner</h1><p className="mt-1 text-sm text-white/40">Scan your dynamic market set. Add any Binance Spot market to the global watchlist and it becomes available here automatically.</p></div>
        ...
        <div className="mt-3 text-[10px] text-white/25">The scanner follows the global market/watchlist state instead of a hardcoded instrument universe.</div>
      </div></div>
    </div>
  </main>;
}
