...
import { ArrowDown, ArrowUp, Filter, ScanSearch } from "lucide-react";
...
export default function ScannerPage() {
  ...
  useEffect(() => {
    ...
    async function scan() {
      const results = await Promise.all(SYMBOLS.map(async (symbol): Promise<ScanRow> => {
        try {
          const params = new URLSearchParams({ symbol, timeframe, pattern_length: String(patternLength), top_k: "10" });
  ...
  }, [timeframe, patternLength]);

  const ranked = useMemo(() => rows
    .filter((row) => row.response)
    .map((row) => {
      const response = row.response!;
      const top = response.matches[0]?.similarity_score ?? 0;
      const stat = response.statistics.find((item) => item.horizon_candles === 60);
      const median = stat?.median_return ?? 0;
      const agreement = median >= 0 ? (stat?.win_rate ?? 0) : 1 - (stat?.win_rate ?? 0);
      return { ...row, response, top, median, agreement, cases: stat?.sample_size ?? 0 };
    })
    .filter((row) => row.top >= minSimilarity && row.cases >= minCases)
    .filter((row) => direction === "all" || (direction === "higher" ? row.median >= 0 : row.median < 0))
    .sort((a, b) => b.top - a.top), [rows, minSimilarity, minCases, direction]);

  const available = rows.filter((row) => row.response).length;
  const filteredOut = available - ranked.length;
  ...
  return <main className="min-h-screen bg-[#070a0f] text-white">
    ...
    <div className="flex"><Sidebar symbol="ETHUSDT" collapsed={collapsed} onCollapsedChange={setCollapsed} onSymbolSelect={(symbol) => { window.location.href = `/scanner?symbol=${symbol}`; }} selectedSymbols={[]} onWatchlistToggle={() => {}} />
      <div className="min-w-0 flex-1"><div className="mx-auto max-w-[1400px] px-4 py-6 sm:px-7 lg:px-10">
        ...
        <div className="mt-3 text-[10px] text-white/25">Pattern match tells you how closely the current shape resembles history. Agreement tells you how consistently those historical cases moved in the same direction.</div>
      </div></div>
    </div>
  </main>;
}
