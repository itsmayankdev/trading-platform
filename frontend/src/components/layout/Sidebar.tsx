    ...
    <aside className={`${collapsed ? "w-[52px]" : "w-[188px]"} hidden shrink-0 border-r border-white/7 bg-[#090d13] transition-[width] duration-200 lg:block`}>
      <div className="sticky top-12 flex h-[calc(100vh-48px)] flex-col">
        <div className="flex h-10 items-center border-b border-white/7 px-2">
          ...
          <button type="button" aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} onClick={() => onCollapsedChange(!collapsed)} className="ml-auto flex h-7 w-7 items-center justify-center rounded-md border border-white/8 text-white/35 transition hover:bg-white/5 hover:text-white/75" title={collapsed ? "Expand sidebar" : "Collapse sidebar"}>
        ...
        <nav className="space-y-0.5 px-2 py-2" aria-label="Research workspace">
          {MENU.map((item) => {
            ...
            return item.live ? (
              <Link key={item.href} href={item.href} className={`flex items-center gap-2 rounded-md px-2.5 py-2 text-[10px] font-medium transition ${active ? "bg-white/[0.06] text-white/85" : "text-white/45 hover:bg-white/[0.03] hover:text-white/70"} ${collapsed ? "justify-center px-0" : ""}`} title={collapsed ? item.label : undefined}>
                <Icon size={14} className={active ? "text-amber-200/80" : "text-white/35"} />
                {!collapsed && <span>{item.label}</span>}
              </Link>
            ) : (
              <div key={item.href} className={`flex items-center gap-2 rounded-md px-2.5 py-2 text-[10px] font-medium text-white/20 ${collapsed ? "justify-center px-0" : ""}`} title={collapsed ? `${item.label} · Coming soon` : undefined}>
                <Icon size={14} className="text-white/15" />
                {!collapsed && <><span>{item.label}</span><span className="ml-auto text-[7px] uppercase tracking-[0.08em] text-white/15">Soon</span></>}
              </div>
            );
          })}
        </nav>
              ...
              return (
                <div key={item.symbol} className={`group flex items-center gap-1.5 rounded-md border px-1.5 py-2 transition ${active ? "border-white/9 bg-white/[0.055]" : "border-transparent hover:bg-white/[0.025]"} ${collapsed ? "justify-center" : ""}`}>
                  <button type="button" onClick={() => onSymbolSelect(item.symbol)} className={`min-w-0 flex-1 text-left ${collapsed ? "flex justify-center" : ""}`} title={collapsed ? `${item.name} (${item.symbol})` : undefined}>
                    <div className="flex items-center gap-1.5"><span className={`h-1.5 w-1.5 shrink-0 rounded-full ${active ? "bg-emerald-400" : "bg-white/15"}`} />{!collapsed && <span className="truncate text-[11px] font-medium text-white/70">{item.symbol.replace("USDT", "")}</span>}</div>
