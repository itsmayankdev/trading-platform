...
export default function Sidebar({ symbol, collapsed, onCollapsedChange, onSymbolSelect }: SidebarProps) {
  ...
  // Compatibility bridge for legacy module selects. The actual market discovery
  // is centralized in the backend and ranked by liquidity; this bridge keeps old
  // module state working until those selects are fully replaced.
  useEffect(() => {
    ...
    const enhance = () => {
      document.querySelectorAll<HTMLSelectElement>("select").forEach((select) => {
        ...
        menu.className = "absolute left-0 top-9 z-50 hidden max-h-72 w-[300px] overflow-auto rounded-md border border-white/10 bg-[#0b1017] p-1 shadow-2xl";
        ...
        const renderResults = (items: Array<{ symbol: string; base_asset?: string | null; quote_asset?: string | null; status?: string | null; quote_volume_24h?: number | null }>) => {
          ...
          items.forEach((item) => {
            ...
            const name = document.createElement("span"); name.className = "block text-[11px] font-medium text-white/75"; name.textContent = `${item.base_asset || item.symbol} · ${item.symbol}`;
            const volume = item.quote_volume_24h == null ? "—" : item.quote_volume_24h >= 1_000_000_000 ? `$${(item.quote_volume_24h / 1_000_000_000).toFixed(1)}B` : item.quote_volume_24h >= 1_000_000 ? `$${(item.quote_volume_24h / 1_000_000).toFixed(1)}M` : `$${item.quote_volume_24h.toFixed(0)}`;
            const meta = document.createElement("span"); meta.className = "block text-[9px] text-white/25"; meta.textContent = `${item.quote_asset || ""} · 24h volume ${volume}`;
        ...
        const search = async () => {
          ...
          try {
            const response = await fetch(`/api/backend/api/v1/instruments?search=${encodeURIComponent(query)}&limit=12&status=TRADING&sort=volume`, { cache: "no-store" });
