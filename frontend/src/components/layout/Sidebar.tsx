...
import { Activity, Bell, BarChart3, ChevronLeft, ChevronRight, FlaskConical, LayoutDashboard, ScanSearch, Star, X, Globe2, ShieldCheck } from "lucide-react";
import Link from "next/link";
import { usePathname } from "next/navigation";

type WatchlistItem = { symbol: string; name: string };
const WATCHLIST: WatchlistItem[] = [{ symbol: "BTCUSDT", name: "Bitcoin" }, { symbol: "ETHUSDT", name: "Ethereum" }, { symbol: "SOLUSDT", name: "Solana" }];
...
export default function Sidebar({ symbol, collapsed, onCollapsedChange, onSymbolSelect, selectedSymbols, onWatchlistToggle }: SidebarProps) {
  ...
  useEffect(() => {
    let alive = true;
    fetch("/api/backend/api/v1/admin/session", { cache: "no-store", credentials: "include" }).then(r => r.json()).then(v => { if (alive) setOwner(v?.authenticated === true && v?.owner === true); }).catch(() => { if (alive) setOwner(false); });
    return () => { alive = false; };
  }, [pathname]);
  return <aside className={`${collapsed ? "w-[52px]" : "w-[188px]"} hidden shrink-0 border-r border-white/7 bg-[#090d13] transition-[width] duration-200 lg:block`}><div className="sticky top-12 flex h-[calc(100vh-48px)] flex-col">
    ...
    <nav className="space-y-0.5 px-2 py-2" aria-label="Research workspace">{MENU.map((item) => { const active = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href); const Icon = item.icon; return <Link key={item.href} href={item.href} className={`flex items-center gap-2 rounded-md px-2.5 py-2 text-[10px] font-medium transition ${active ? "bg-white/[0.06] text-white/85" : "text-white/45 hover:bg-white/[0.03] hover:text-white/70"} ${collapsed ? "justify-center px-0" : ""}`} title={collapsed ? item.label : undefined}><Icon size={14} className={active ? "text-amber-200/80" : "text-white/35"} />{!collapsed && <span>{item.label}</span>}</Link>; })}</nav>
    <div className="px-2"><Link href="/favorites" className={`flex items-center gap-2 rounded-md px-2.5 py-2 text-[10px] font-medium transition ${pathname.startsWith("/favorites") ? "bg-white/[0.06] text-white/85" : "text-white/45 hover:bg-white/[0.03] hover:text-white/70"} ${collapsed ? "justify-center px-0" : ""}`} title={collapsed ? "Favorites" : undefined}><Star size={14} className={pathname.startsWith("/favorites") ? "text-amber-200/80" : "text-white/35"} fill={pathname.startsWith("/favorites") ? "currentColor" : "none"} />{!collapsed && <span>Favorites</span>}</Link></div>
    {owner && <div className="mt-2 border-y border-amber-300/10 px-2 py-2"><Link href="/admin" className={`flex items-center gap-2 rounded-md px-2.5 py-2 text-[10px] font-semibold transition ${pathname.startsWith("/admin") ? "bg-amber-300/10 text-amber-100" : "text-amber-200/55 hover:bg-amber-300/[0.05] hover:text-amber-100"} ${collapsed ? "justify-center px-0" : ""}`} title={collapsed ? "Admin Control" : undefined}><ShieldCheck size={14}/>{!collapsed && <><span>Admin Control</span><span className="ml-auto text-[7px] uppercase tracking-[0.08em] text-amber-200/35">Owner</span></>}</Link></div>}
