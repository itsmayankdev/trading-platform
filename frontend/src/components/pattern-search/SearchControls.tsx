...
type Instrument = { symbol: string; base_asset: string; quote_asset: string; status: string; spot_trading_allowed: boolean };
  ...
  return <section className="relative z-30 border-b border-white/[0.07] bg-[#080b10] px-3 py-2 sm:px-4">
    <div className="flex w-full min-w-0 items-center gap-2">
      <div className={`relative min-w-0 flex-1 sm:flex-none ${marketFocused ? "sm:w-[260px]" : "sm:w-[220px]"}`}>
        ...
        {marketFocused && <><button type="button" aria-label="Close market suggestions" className="fixed inset-0 -z-10 cursor-default" onClick={() => setMarketFocused(false)} /><div className="absolute left-0 top-[calc(100%+7px)] w-[min(360px,calc(100vw-24px))] overflow-hidden rounded-xl border border-white/[0.12] bg-[#0b0f15] p-1.5 shadow-2xl shadow-black/50">
          ...
          {ranked.length ? ranked.slice(0, 12).map((item, index) => <button key={item.symbol} type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => selectSymbol(item.symbol)} className="flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left transition hover:bg-white/[0.055]">
            ...
            <span className="flex items-center gap-2"><span className="text-[9px] text-white/30">Listed</span>{item.symbol === symbol && <Check size={14} className="shrink-0 text-emerald-400" />}</span>
          </button>) : <div className="px-2.5 py-4 text-xs text-white/35">{marketLoading ? "Finding markets…" : marketQuery ? "No matching Binance market." : "Type a symbol or coin name to search."}</div>}
        </div></>}
      </div>
