...
const TIMEFRAMES = [
  { value: "1m", label: "1m", detail: "1 minute" },
...
export default function SearchControls({ symbol, timeframe, patternLength, topK, loading, liveQuote, onSymbolChange, onTimeframeChange, onPatternLengthChange, onTopKChange }: SearchControlsProps) {
  ...
  function selectMarket(value: string) { const normalized = normalizeMarketSymbol(value); if (normalized) onSymbolChange(normalized); }
  function selectTimeframe(value: string) { window.dispatchEvent(new CustomEvent("market-memory:data-change", { detail: { kind: "timeframe", value } })); onTimeframeChange(value); }
  function selectPatternLength(value: string) { window.dispatchEvent(new CustomEvent("market-memory:data-change", { detail: { kind: "pattern", value } })); onPatternLengthChange(value); }
  const currentLabel = symbol.includes("/") ? symbol : symbol.endsWith("USDT") ? `${symbol.slice(0, -4)}/USDT` : symbol;
  return <section className="relative z-30 border-b border-white/[0.07] bg-[#080b10] px-3 py-2 sm:px-4"><div className="flex w-full min-w-0 items-center gap-2">
    <div className="relative min-w-0 flex-1 sm:flex-none sm:w-[260px]"><MarketSelector value={symbol} onChange={selectMarket} className="w-full" /></div>
    <div className="flex h-9 shrink-0 items-center gap-2 rounded-lg border border-white/[0.08] bg-[#0b0f15] px-2.5"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400"/><span className="max-w-[100px] truncate text-[9px] font-semibold uppercase tracking-[0.05em] text-white/45">{currentLabel}</span><span className="font-mono text-xs font-semibold tabular-nums text-white/90">{market ? formatPrice(market.price) : "—"}</span>{market && <span className={`font-mono text-[10px] tabular-nums ${market.change >= 0 ? "text-emerald-400" : "text-rose-400"}`}>{market.change >= 0 ? "+" : ""}{market.change.toFixed(2)}%</span>}</div>
    <div className="hidden h-6 w-px shrink-0 bg-white/[0.08] sm:block"/>
    <div className="flex h-9 shrink-0 items-center rounded-lg border border-white/[0.08] bg-[#0b0f15] p-0.5" aria-label="Timeframe">{TIMEFRAMES.map(item => <PillButton key={item.value} active={timeframe === item.value} onClick={() => selectTimeframe(item.value)} title={item.detail}>{item.label}</PillButton>)}</div>
    <div className="hidden h-9 shrink-0 items-center gap-0.5 rounded-lg border border-white/[0.08] bg-[#0b0f15] p-0.5 md:flex" aria-label="Pattern length">{PATTERN_LENGTHS.map(value => <PillButton key={value} active={patternLength === value} onClick={() => selectPatternLength(value)} title={`${value} candles`}>{value}</PillButton>)}<span className="px-1.5 text-[9px] uppercase tracking-[0.1em] text-white/25">candles</span></div>
