...
export default function ForwardPathChart({ paths, selectedMatchIndex = 0, onSelectedMatchIndexChange }: { paths?: ForwardPath[]; selectedMatchIndex?: number; onSelectedMatchIndexChange?: (index: number) => void }) {
  const [hoveredMatch, setHoveredMatch] = useState<ForwardPath | null>(null);
  ...
  return (
    <section className="panel overflow-hidden">
      ...
      <div className="relative px-2 pb-1 pt-1 sm:px-3"><svg viewBox={`0 0 ${width} ${height}`} className="h-[235px] w-full" role="img" aria-label="Normalized historical forward paths">
        ...
        {safePaths.map((path) => { const active = path.match_index === selectedMatchIndex + 1; const hovered = hoveredMatch?.match_index === path.match_index; return <g key={path.match_index} onMouseEnter={() => setHoveredMatch(path)} onMouseLeave={() => setHoveredMatch(null)} onClick={() => onSelectedMatchIndexChange?.(path.match_index - 1)} className="cursor-pointer"><polyline points={linePoints(path.values)} fill="none" stroke={active || hovered ? "rgba(245,158,11,0.95)" : "rgba(255,255,255,0.11)"} strokeWidth={active ? "2.5" : hovered ? "2" : "1"} /><polyline points={linePoints(path.values)} fill="none" stroke="transparent" strokeWidth="10" /></g>; })}
        <polyline points={linePoints(median)} fill="none" stroke="rgba(255,255,255,0.82)" strokeWidth="2" pointerEvents="none" /><text x={x(0) + 5} y={pad.top - 7} fontSize="8" fill="rgba(245,158,11,0.75)" fontFamily="sans-serif">MATCH POINT</text><text x={x(maxSteps - 1)} y={height - 10} textAnchor="end" fontSize="8" fill="rgba(255,255,255,0.25)" fontFamily="sans-serif">+{maxSteps - 1} candles</text><text x={pad.left} y={height - 10} fontSize="8" fill="rgba(255,255,255,0.25)" fontFamily="sans-serif">0</text>
      </svg>{hoveredMatch && <div className="pointer-events-none absolute right-5 top-3 z-10 w-[170px] rounded-md border border-white/10 bg-[#090d13]/95 px-2.5 py-2 shadow-2xl backdrop-blur-sm"><div className="text-[8px] font-semibold uppercase tracking-[0.12em] text-white/35">Historical match {String(hoveredMatch.match_index).padStart(2, "0")}</div><div className="mt-1 flex items-baseline justify-between"><span className="text-[8px] uppercase text-white/25">Similarity</span><span className="font-mono text-[10px] text-white/70">{hoveredMatch.similarity_score.toFixed(2)}%</span></div><div className="mt-0.5 flex items-baseline justify-between"><span className="text-[8px] uppercase text-white/25">Return @ +60</span><span className={`font-mono text-[10px] ${((hoveredMatch.values.at(-1) ?? 0) >= 0) ? "text-emerald-300" : "text-rose-300"}`}>{formatPct(hoveredMatch.values.at(-1) ?? 0)}</span></div><div className="mt-1 border-t border-white/7 pt-1 text-[8px] text-white/25">Click path to inspect this match</div></div>}</div>
