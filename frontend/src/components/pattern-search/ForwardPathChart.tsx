...
export default function ForwardPathChart({ paths }: { paths?: ForwardPath[] }) {
  ...
  const allValues = columns.flatMap((values) => values);
  if (!allValues.length) return null;
  const low = Math.min(-0.02, ...allValues);
  const high = Math.max(0.02, ...allValues);
  ...
  const finalMedian = median.at(-1) ?? 0;
  const finalUp = safePaths.filter((path) => (path.values.at(-1) ?? 0) >= 0).length;
  ...
  return (
    <section className="panel overflow-hidden">
      <div className="flex items-center justify-between border-b border-white/7 px-4 py-3">
        ...
        <div className="hidden items-center gap-3 text-[8px] uppercase tracking-[0.1em] text-white/25 sm:flex"><span>25–75% range</span><span>Median path</span><span>{safePaths.length} matches</span></div>
      </div>
      <div className="px-2 pb-2 pt-1 sm:px-3">
        <svg viewBox={`0 0 ${width} ${height}`} className="h-[250px] w-full" role="img" aria-label="Normalized historical forward paths">
          ...
          {safePaths.map((path) => <polyline key={path.match_index} points={linePoints(path.values)} fill="none" stroke="rgba(255,255,255,0.13)" strokeWidth="1" />)}
      ...
      <div className="grid grid-cols-3 border-t border-white/7 text-center">
        <div className="px-2 py-2"><div className="text-[8px] uppercase tracking-[0.12em] text-white/25">Median @ +60</div><div className={`mt-0.5 font-mono text-xs ${finalMedian >= 0 ? "text-emerald-400" : "text-rose-400"}`}>{formatPct(finalMedian)}</div></div>
        ...
        <div className="px-2 py-2"><div className="text-[8px] uppercase tracking-[0.12em] text-white/25">Path agreement</div><div className="mt-0.5 font-mono text-xs text-white/55">{finalMedian >= 0 ? `${finalUp}/${safePaths.length} up` : `${safePaths.length - finalUp}/${safePaths.length} down`}</div></div>
      </div>
    </section>
  );
}
