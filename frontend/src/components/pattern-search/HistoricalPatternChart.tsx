...
function timeframeSeconds(timeframe: string) { if (timeframe === "1h") return 3600; if (timeframe === "15m") return 900; return 300; }
...
function HistoricalChartCanvas({ symbol, timeframe, patternLength, match, highlightLocked, dashboardFullscreen, onFullscreenToggle }: { symbol: string; timeframe: string; patternLength: number; match: Match; highlightLocked: boolean; dashboardFullscreen: boolean; onFullscreenToggle: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/backend/api/v1/candles?${params.toString()}`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        ...
        setCandles(next); setStatus("ready");
      })
      .catch((error: unknown) => {
        if (error instanceof DOMException && error.name === "AbortError") return;
        if (!controller.signal.aborted) { setCandles([]); setStatus("error"); }
      });
  ...
  useEffect(() => {
    ...
    const chart = createChart(container, {
      width: container.clientWidth, height: Math.max(300, container.clientHeight),
      ...
      handleScroll: highlightLocked ? false : true, handleScale: highlightLocked ? false : true,
    });
    ...
    return () => {
      disposed = true; resizeObserver.disconnect(); chart.unsubscribeCrosshairMove(handleCrosshairMove); hideTooltip();
      if (chartRef.current === chart) chartRef.current = null;
      chart.remove();
    };
    // Chart is rebuilt when loaded candle data changes.
  ...
  return (
    <div ref={containerRef} onMouseLeave={hideTooltip} className={`group relative w-full ${dashboardFullscreen ? "min-h-0 flex-1" : "h-[380px]"}`}>
      ...
      {highlightLocked && <div ref={patternBoxRef} className="pointer-events-none absolute bottom-[28px] top-[8px] z-10 border border-amber-300/75 bg-amber-300/[0.07]" title={`Highlighted area = the matched ${patternLength}-candle pattern. Hover a candle to see OHLC and match context.`}><span className="pointer-events-none absolute left-1 top-1 rounded bg-amber-300/90 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-[0.1em] text-black">Matched {patternLength}</span></div>}
...
export default function HistoricalPatternChart({ symbol, timeframe, patternLength, matches, highlightLocked, dashboardFullscreen, onFullscreenToggle }: Props) {
  ...
  useEffect(() => { setIndex((value) => Math.min(value, Math.max(0, matches.length - 1))); }, [matches.length]);
