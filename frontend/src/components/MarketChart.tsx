...
import { getMarketCandles, prefetchMarketCandles, type CachedCandle } from "@/lib/marketCache";
...
export default function MarketChart({ symbol, timeframe, patternLength, highlightLocked, dashboardFullscreen, onFullscreenToggle, onPin }: MarketChartProps) {
  const containerRef = useRef<HTMLDivElement | null>(null); const chartRef = useRef<IChartApi | null>(null); const patternBoxRef = useRef<HTMLDivElement | null>(null); const tooltipRef = useRef<HTMLDivElement | null>(null); const candlesRef = useRef<Candle[]>([]);
  ...
  useEffect(() => {
    ...
    // Stale-while-revalidate: cached candles paint synchronously, then the same
    // request refreshes quietly in the background. A market switch therefore
    // never waits for the network before the chart becomes visible.
    const cached = getMarketCandles(symbol, timeframe);
    if (cached) render(cached);
    void prefetchMarketCandles(symbol, timeframe, Math.max(patternLength * 2, 120)).then((fresh) => render(fresh)).catch((error) => { if (!disposed) console.error("Failed to load candles:", error); });

    return () => { disposed = true; resizeObserver.disconnect(); chart.unsubscribeCrosshairMove(handleCrosshairMove); hideTooltip(); if (chartRef.current === chart) chartRef.current = null; chart.remove(); };
