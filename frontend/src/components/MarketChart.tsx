...
export default function MarketChart({ symbol, timeframe, patternLength, highlightLocked, dashboardFullscreen, onFullscreenToggle, onPin }: MarketChartProps) {
  ...
  useEffect(() => {
    ...
    function handleCrosshairMove(param: MouseEventParams<Time>) {
      ...
      const candle = candleByTimeRef.current.get(Number(param.time));
      ...
      const matched = candlesRef.current.length >= currentPatternLength && candlesRef.current.indexOf(candle) >= candlesRef.current.length - currentPatternLength;
      ...
      const x = Math.min(Math.max(param.point.x + 12, 8), Math.max(8, host.clientWidth - 142));
      const y = Math.min(Math.max(param.point.y - 18, 8), Math.max(8, host.clientHeight - 82));
  ...
  return <div ref={containerRef} onMouseLeave={hideTooltip} className={`group relative w-full ${dashboardFullscreen ? "min-h-0 flex-1" : "h-[380px]"}`}>
    ...
    <div onMouseEnter={hideTooltip} className="pointer-events-none absolute right-2 top-2 z-30 flex gap-1 opacity-0 transition-opacity group-hover:pointer-events-auto group-hover:opacity-100">
      <button type="button" aria-label="Pin current chart pattern" title="Pin pattern to Favorites" onClick={onPin} className="pointer-events-auto flex h-7 items-center gap-1 rounded border border-white/10 bg-[#090d13]/90 px-2 text-[8px] font-semibold uppercase tracking-[0.1em] text-white/60 shadow-lg backdrop-blur hover:text-amber-200"><Pin size={11} /> Pin</button>
    ...
    {highlightLocked && <div ref={patternBoxRef} className="pointer-events-none absolute bottom-[28px] top-[8px] z-10 border border-amber-300/75 bg-amber-300/[0.07]"><span className="pointer-events-none absolute left-1 top-1 rounded bg-amber-300/90 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-[0.1em] text-black">Matched {patternLength}</span></div>}
