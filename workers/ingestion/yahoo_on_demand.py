...
# Yahoo intraday history is provider-limited, so the global-market lane has its
# own bounded policy. It never changes Binance's ingestion policy or jobs.
_INTRADAY_FULL_DAYS = 60
_DAILY_FULL_DAYS = 3650
_FAST_INTRADAY_DAYS = 7
_FAST_DAILY_DAYS = 365
_TIMEFRAMES = {"1m", "3m", "5m", "15m", "30m", "1h", "1d"}
_MAX_WORKERS = 2
...
def _coverage(symbol: str, timeframe: str) -> tuple[datetime | None, datetime | None, int]:
    with SessionLocal() as db:
        row = db.execute(
            select(Candle.timestamp).join(Instrument, Instrument.id == Candle.instrument_id).where(
        ...
        count = db.execute(
            select(Candle.id).join(Instrument, Instrument.id == Candle.instrument_id).where(
                Instrument.symbol == symbol,
                Instrument.provider == "yahoo",
                Candle.timeframe == timeframe,
            )
...
def _run_full(symbol: str, timeframe: str) -> None:
    ...
    if max_time is not None and min_time is not None and min_time <= desired_start and max_time >= end - timedelta(days=2):
        return
    if start >= end:
        return
    _fetch_and_store(symbol, timeframe, start, end)
...
def ensure_yahoo_market_data(symbol: str, timeframe: str, minimum_candles: int) -> dict[str, object]:
    ...
    return {
        ...
        "provider": "yahoo",
        "intraday_history_limit_days": _INTRADAY_FULL_DAYS,
    }
