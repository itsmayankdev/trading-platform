...
class IngestionScheduler:
    ...
    def __init__(self, policy_path: Path = DEFAULT_POLICY_PATH) -> None:
        ...
        raw_tiers = list(config.get("tiers", []))
        # Usage promotion is a platform capability, not a frontend setting. Keep a
        # safe default so older policy files gain promotion without a migration.
        if not any(item.get("top_by_usage") is not None for item in raw_tiers):
            raw_tiers.append({"name": "promoted", "priority": 20, "symbols": [], "top_by_usage": 10, "history_days": 90})
        self.tiers = tuple(
            Tier(
                name=item["name"],
                priority=int(item["priority"]),
                symbols=frozenset(symbol.upper() for symbol in item.get("symbols", [])),
                top_by_quote_volume=int(item["top_by_quote_volume"]) if item.get("top_by_quote_volume") is not None else None,
                top_by_usage=int(item["top_by_usage"]) if item.get("top_by_usage") is not None else None,
    ...
    def reconcile(self) -> dict[str, int]:
        discovery = InstrumentRegistrySync().sync()
        ...
        for tier in sorted(self.tiers, key=lambda item: item.priority):
            ...
            if tier.top_by_usage is not None:
                ranked_usage = sorted(((symbol, usage_rank.get(symbol, 0)) for symbol in eligible_symbols if symbol not in assigned_symbols and usage_rank.get(symbol, 0) > 0), key=lambda item: (-item[1], item[0]))
                symbols.update(symbol for symbol, _ in ranked_usage[:tier.top_by_usage])
            if tier.top_by_quote_volume is not None:
                ranked_volume = sorted(((symbol, ticker_map.get(symbol, 0.0)) for symbol in eligible_symbols if symbol not in assigned_symbols), key=lambda item: item[1], reverse=True)
            ...
            planner = IngestionPlanner(timeframes=self.timeframes, history_days=tier.history_days)
            tier_plans = [plan for plan in planner.build_plans() if plan.symbol in symbols]
            plans.extend((tier.priority, ticker_map.get(plan.symbol, 0.0), usage_rank.get(plan.symbol, 0), plan) for plan in tier_plans)
        plans.sort(key=lambda item: (item[0], -item[2], -item[1], item[3].symbol, item[3].timeframe, item[3].reason))
        ...
        return {"discovered": discovery["discovered"], "eligible": discovery["eligible"], "disabled": discovery["disabled"], "usage_markets": len(usage_rank), "plans": len(plans), "selected": len(selected), "queued": queued}
    ...
    def _eligible_symbols() -> set[str]:
        ...
        with SessionLocal() as db:
            rows = db.execute(text("""
                ...
                WHERE is_enabled = TRUE AND is_listed = TRUE AND is_spot_trading_allowed = TRUE
                  AND exchange_status = 'TRADING' AND exchange = 'binance' AND provider = 'binance' AND quote_asset = 'USDT'
            """)).scalars().all()
    ...
    def _usage_rank() -> dict[str, int]:
        ...
        with SessionLocal() as db:
            rows = db.execute(text("""
                ...
                WHERE event_type = 'market_selected' AND created_at >= now() - interval '7 days'
                  AND metadata_json ->> 'symbol' IS NOT NULL
                GROUP BY upper(metadata_json ->> 'symbol')
                ORDER BY uses DESC, symbol LIMIT 100
            """)).all()
...
def run_scheduler(interval_seconds: int, run_once: bool = False) -> None:
    ...
    while True:
        ...
        try:
            ...
            print("Scheduler reconciliation: " + ", ".join(f"{key}={value}" for key, value in result.items()), flush=True)
