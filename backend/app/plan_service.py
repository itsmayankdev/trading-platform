...
def record_search(db: Session, user: AdminUser, symbol: str, timeframe: str, pattern_length: int, top_k: int) -> None:
    db.add(AdminUsageEvent(user_id=user.id, event_type="pattern_search", module="market_memory", path="/api/v1/pattern-search", metadata_json={"symbol": symbol, "timeframe": timeframe, "pattern_length": pattern_length, "top_k": top_k}))
    db.commit()
