...
from backend.app.models.admin import AdminPlan, AdminRole, AdminUsageEvent, AdminUser

PUBLIC_PLAN_CODES = {"free", "pro", "enterprise"}
TRIAL_DAYS = 3
TRIAL_LIMITS = {"pro": {"searches_per_day": 20, "max_matches": 10}, "enterprise": {"searches_per_day": 30, "max_matches": 15}}
...
def effective_plan(db: Session, user: AdminUser) -> AdminPlan:
    ...
    if user.plan_status == "trialing" and user.trial_ends_at:
        ...
        if free:
            ...
            db.commit(); user.plan = free; return free
    if user.plan:
        return user.plan
    free = db.scalar(select(AdminPlan).where(AdminPlan.code == "free", AdminPlan.active.is_(True)))
    if not free:
        raise RuntimeError("Free plan is not configured")
    user.plan_id = free.id; user.plan_status = "active"; user.updated_at = now; db.commit(); user.plan = free; return free

def effective_limits(user: AdminUser, plan: AdminPlan) -> dict:
    limits = dict(plan.limits_json or {})
    if user.plan_status == "trialing": limits.update(TRIAL_LIMITS.get(plan.code, {}))
    return limits

def _assign_registration_role(db: Session, user: AdminUser, plan_code: str) -> None:
...
def activate_registration_plan(db: Session, user: AdminUser, requested_code: str) -> AdminPlan:
    ...
    if code != "free" and plan.trial_days > 0:
        user.plan_status = "trialing"; user.trial_plan_id = plan.id; user.trial_started_at = now; user.trial_ends_at = now + timedelta(days=plan.trial_days)
    user.plan = plan; _assign_registration_role(db, user, code); return plan
