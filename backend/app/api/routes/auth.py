...
from backend.app.services.plan_service import activate_registration_plan, effective_plan, search_usage

router=APIRouter(prefix="/api/v1/auth",tags=["auth"])

def _load_user(db:Session,user_id:int):
    return db.scalar(select(AdminUser).options(
        selectinload(AdminUser.roles).selectinload(AdminRole.permissions),
        selectinload(AdminUser.permissions),
        selectinload(AdminUser.module_overrides),
        selectinload(AdminUser.plan),
        selectinload(AdminUser.trial_plan),
    ).where(AdminUser.id==user_id))

def _serialize(user:AdminUser,db:Session)->dict:
    plan=effective_plan(db,user)
    permissions={p.code for role in user.roles for p in role.permissions}|{p.code for p in user.permissions}
    modules={m.module:m.enabled for m in user.module_overrides}
    used=search_usage(db,user.id)
    limits=plan.limits_json or {}
    return {
        "id":user.id,"email":user.email,"display_name":user.display_name,"first_name":user.first_name,"last_name":user.last_name,"status":user.status,
        "plan":{"id":plan.id,"code":plan.code,"name":plan.name,"description":plan.description,"price_cents":plan.price_cents,"features":plan.features_json or [],"limits":limits},
        ...
        "usage":{"searches_today":used,"search_limit":limits.get("searches_per_day")},
        "roles":[{"id":r.id,"code":r.code,"name":r.name} for r in user.roles],"permissions":sorted(permissions),"modules":modules,
        "access_version":user.access_version,"created_at":user.created_at.isoformat() if user.created_at else None,"last_seen_at":user.last_seen_at.isoformat() if user.last_seen_at else None,
    }
...
def register(response:Response,body:dict=Body(...),db:Session=Depends(get_db)):
    ...
    plan_code=str(body.get("plan_code","free")).strip().lower()
    ...
    try:
        user=AdminUser(email=email,display_name=name,first_name=first,last_name=last,password_hash=hash_password(password),status="active")
        activate_registration_plan(db,user,plan_code)
        role=db.scalar(select(AdminRole).where(AdminRole.code=="analyst"))
...
def session(request:Request,response:Response,db:Session=Depends(get_db)):
    ...
    if not user:
        clear_user_cookie(response);return {"authenticated":False}
    user.last_seen_at=datetime.now(timezone.utc);db.commit();user=_load_user(db,user.id)
    if user is None:
        clear_user_cookie(response);return {"authenticated":False}
