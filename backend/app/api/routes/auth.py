...
def _load_user(db: Session, user_id: int) -> AdminUser | None:
...
def register(body: dict = Body(...), response: Response = None, db: Session = Depends(get_db)):
    ...
    user = AdminUser(email=email, display_name=name, password_hash=hash_password(password), status="active")
    plan = db.scalar(select(AdminPlan).where(AdminPlan.code == "free", AdminPlan.active.is_(True)))
    role = db.scalar(select(AdminRole).where(AdminRole.code == "viewer"))
...
def session(request: Request, db: Session = Depends(get_db)):
    ...
    user.last_seen_at = datetime.now(timezone.utc)
    db.commit()
    return {"authenticated": True, **_serialize(user)}


@router.get("/me")
def me(request: Request, db: Session = Depends(get_db)):
    return _serialize(require_user(request, db))
