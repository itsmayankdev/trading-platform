...
def replay_search(
    ...
    rows = list(db.execute(
        ...
        .where(
            ...
            Candle.timestamp <= replay_time,
        )
        .order_by(Candle.timestamp.asc())
    ).all())
    if len(rows) < pattern_length + 1:
        raise HTTPException(status_code=400, detail="Not enough completed candles before replay time")
    ...
    current_start = current_rows[0].timestamp
    current_end = current_rows[-1].timestamp
    current = PatternWindow(
        symbol=symbol,
        timeframe=timeframe,
        start_time=current_start,
        end_time=current_end,
        candles=tuple(CandlePoint(timestamp=r.timestamp, open=r.open, high=r.high, low=r.low, close=r.close, volume=r.volume) for r in current_rows),
    )
    matches = PatternRanker().rank_numerical_v1(
        current=current,
        store=store,
        top_k=top_k,
    ...
    return {
        ...
        "current_pattern": {"start_time": current_start, "end_time": current_end},
        "historical_candles_available": len(rows),
        "matches": [
            {"start_time": match.start_time, "end_time": match.end_time, "similarity_score": round(match.similarity_score * 100, 4)}
