...
def _serialize_user(user:AdminUser)->dict:
    ...
    return {"id":user.id,"email":user.email,"display_name":user.display_name,"first_name":user.first_name,"last_name":user.last_name,"status":user.status,"plan":{"id":user.plan.id,"code":user.plan.code,"name":user.plan.name} if user.plan else None,"roles":[{"id":r.id,"code":r.code,"name":r.name} for r in user.roles],"permissions":access["permissions"],"modules":access["modules"],"access_version":user.access_version,"last_seen_at":user.last_seen_at,"created_at":user.created_at,"subscription_started_at":user.subscription_started_at,"subscription_ends_at":user.subscription_ends_at}
...
def users(request:Request,q:str="",status:str="",plan_id:int|None=None,enrolled_from:str="",enrolled_to:str="",ends_from:str="",ends_to:str="",limit:int=50,offset:int=0,db:Session=Depends(get_db)):
    ...
    if plan_id is not None:stmt=stmt.where(AdminUser.plan_id==plan_id)
    dt=_parse_dt(enrolled_from)
    if dt:stmt=stmt.where(AdminUser.subscription_started_at>=dt)
    dt=_parse_dt(enrolled_to)
    if dt:stmt=stmt.where(AdminUser.subscription_started_at<=dt)
    dt=_parse_dt(ends_from)
    if dt:stmt=stmt.where(AdminUser.subscription_ends_at>=dt)
    dt=_parse_dt(ends_to)
    if dt:stmt=stmt.where(AdminUser.subscription_ends_at<=dt)
