...
def pattern_search(symbol: str = Query(default="ETHUSDT", min_length=1, max_length=50), timeframe: str = Query(default="5m", min_length=1, max_length=10), pattern_length: int = Query(default=45, ge=5, le=500), top_k: int = Query(default=10, ge=1, le=50), request: Request = None, db: Session = Depends(get_db)):
    ...
    try:
        return service.search(instrument_id=instrument.id, symbol=symbol, timeframe=timeframe, pattern_length=pattern_length, top_k=top_k)
