...
def get_candles(request: Request, symbol: str = Query(default="ETHUSDT", min_length=1, max_length=50), timeframe: str = Query(default="5m", min_length=1, max_length=10), limit: int = Query(default=500, ge=1, le=5000), start_time: datetime | None = Query(default=None), end_time: datetime | None = Query(default=None), db: Session = Depends(get_db)):
    ...
    if start_time is not None and end_time is not None and start_time > end_time: raise HTTPException(status_code=400, detail="start_time must be before end_time")
    ...
    if instrument.exchange != "binance" or instrument.provider != "binance" or not instrument.is_listed or not instrument.is_spot_trading_allowed:
        raise HTTPException(status_code=400, detail=f"Instrument is not an active Binance Spot market: {symbol}")

    # Every module uses this endpoint for chart data. Keep first-use behavior
    # centralized here so new markets do not require module-specific warmup code.
    try:
        ...
        # Preserve the normal DB-backed response when background/seed ingestion
        # cannot reach the provider; callers still get whatever history exists.
